...
def get_img(UDPSocket_cam, params_cam):
    ...
    if params_cam["SOCKET_TYPE"] == 'JPG':
        ...
        img_head = np.zeros(int(params_cam["HEIGHT"] / 30), )
        UnitBlockSize_cam = int(params_cam["WIDTH"] * 30 * 3 + 8)
...
def cam_calib(params_calib):
    ...
    TEST_IMAGE_PATHS = [os.path.join(PATH_TO_TEST_IMAGES_DIR, str(i).zfill(2) + '.jpg') for i in
                        range(1, 1 + params_calib["# of img"])]
